    ...
    public static ArrayList<Macro> consultaMacroDpdAsc(String agente,String dpd,String algoritmo){
        ...
        try{
            ...
            ps=con.prepareStatement("SELECT \n" +
                ...
                "From (\n" +
                "SELECT \n" +
                "	  FORMAT([Fecha_interaccion], 'HH:mm:ss') AS HoraMinutoSegundo\n" +
                "      ,[Cliente]\n" +
                "	  ,ROW_NUMBER() OVER (PARTITION BY [Cliente] ORDER BY [Fecha_interaccion] DESC) AS DUPLICADOS\n" +
                "  FROM [DC REPORT].[dbo].[Mitrol_int]\n" +
                "Where CONVERT(DATE,[Fecha_interaccion]) = CONVERT(DATE,GETDATE())\n" +
                "and [Campaña] like('%W%')\n" +
                "and [Campaña]  not in ('WA Telemarketing Vivus Piloto')\n" +
                ") as W\n" +
                "where W.DUPLICADOS = 1) AS m \n" +
                "ON m.Cliente = a.[TEL]\n" +
                "\n" +
                ...
                "  --AND m.HoraMinutoSegundo IS NOT NULL\n" +
                "\n" +
